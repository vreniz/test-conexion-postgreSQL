...
HOST = "localhost"
PUERTO = 5432
BASE_DATOS = "cruises_dw"  # Cambiado para reflejar el dominio del proyecto
...
def _validar_star_schema(ships, countries, suites, packages, booking_sources, bookings_fact):
    """Valida el contrato de datos del esquema estrella antes de escribir."""
    
    # 1. Definición de contratos (columnas esperadas)
    contratos = {
        "ships_dim": {"id_ship", "ship_name", "company", "route_type", "total_cabins", "total_crew"},
        ...
        "bookings_fact": {
            "booking_id", "travel_date", "booking_date", "id_ship", "id_country", 
            "id_suite", "id_package", "id_booking_source", "lead_time_days", 
            "nights_of_stay", "booked_cabins", "passengers_on_booking", 
            "cycle_occupancy_percentage", "total_reservation_income_usd", 
            "average_daily_guest_spending_usd", "satisfaction_score"
        }
    }
    ...
    for nombre, df, esperadas in [
        ...
        ("booking_sources_dim", booking_sources, contratos["booking_sources_dim"]),
        ("bookings_fact", bookings_fact, contratos["bookings_fact"])
    ]:
        if set(df.columns) != esperadas:
            raise ValueError(f"Columnas inesperadas en {nombre}: {df.columns.tolist()}")

    # 3. Validar Llaves Primarias únicas en Dimensiones
    for nombre, df, col_id in [
        ...
        if df[col_id].duplicated().any():
            raise ValueError(f"Hay IDs duplicados en la columna {col_id} de {nombre}.")

    # 4. Validar que no existan huérfanos (Nulos en llaves foráneas de la Fact)
    llaves_foraneas = ["id_ship", "id_country", "id_suite", "id_package", "id_booking_source"]
    for fk in llaves_foraneas:
        if bookings_fact[fk].isna().any():
            raise ValueError(f"Error Crítico: Hay registros en la tabla de hechos con {fk} nulo.")


def cargar_esquema_estrella(ships, countries, suites, packages, booking_sources, bookings_fact):
    """Carga el modelo multidimensional completo en PostgreSQL dentro de una transacción."""
    
    # Ejecutar validaciones previas
    _validar_star_schema(ships, countries, suites, packages, booking_sources, bookings_fact)
    ...
    with engine.begin() as conexion:
        ...
        # Eliminar primero la tabla de hechos (hija) debido a las restricciones FK
        print("Limpiando tablas previas...")
        conexion.execute(text("DROP TABLE IF EXISTS bookings_fact;"))
        conexion.execute(text("DROP TABLE IF EXISTS ships_dim;"))
        conexion.execute(text("DROP TABLE IF EXISTS countries_dim;"))
        conexion.execute(text("DROP TABLE IF EXISTS suites_dim;"))
        conexion.execute(text("DROP TABLE IF EXISTS packages_dim;"))
        conexion.execute(text("DROP TABLE IF EXISTS booking_sources_dim;"))
        ...
        conexion.execute(text("""
            CREATE TABLE bookings_fact (
                ...
                -- Validaciones de Negocio (Checks)
        ...
        print("Creando índices de optimización para Power BI...")
        # Índices en la tabla de hechos para acelerar los Joins del modelo en Estrella
        for fk in ["id_ship", "id_country", "id_suite", "id_package", "id_booking_source", "travel_date", "booking_date"]:
            conexion.execute(text(f"CREATE INDEX idx_fact_{fk} ON bookings_fact({fk});"))

        print("Insertando datos en las dimensiones...")
        # Carga masiva ordenada (Primero dimensiones, luego la tabla de hechos)
        ships.to_sql("ships_dim", conexion, if_exists="append", index=False, method="multi")
        countries.to_sql("countries_dim", conexion, if_exists="append", index=False, method="multi")
        suites.to_sql("suites_dim", conexion, if_exists="append", index=False, method="multi")
        packages.to_sql("packages_dim", conexion, if_exists="append", index=False, method="multi")
        booking_sources.to_sql("booking_sources_dim", conexion, if_exists="append", index=False, method="multi")

        print("Insertando datos en la tabla de hechos (esto puede tardar unos segundos)...")
        bookings_fact.to_sql("bookings_fact", conexion, if_exists="append", index=False, method="multi")

       # 3. Validación final de conteos coincidentes
